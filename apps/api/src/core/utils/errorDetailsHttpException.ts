...
import { formatValidationErrorToString } from './formatValidationErrorToString';
...
export function toErrorDetailsHttpException(
  ...
  if (exception instanceof DocumentNotExistsError) {
    return ErrorDetails.from(
      StatusCode.NOT_FOUND,
      `${exception.docName} not found`
    )
      .error(exception)
      .toHttpException();
  }
  if (exception instanceof InvalidArgumentError) {
    return ErrorDetails.from(StatusCode.INVALID_ARGUMENT, exception.message)
      .badRequest({
        fieldViolations: [
          { field: exception.argName, description: exception.message },
        ],
      })
      .error(exception)
      .toHttpException();
